...
class push2talk_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct( false, __( 'push2talk', 'push2talk' ) );
    }
 
    function widget( $args, $instance ) {
        $html = file_get_contents(plugin_dir_path( __DIR__ ) . 'widget/build/index.html');
    ...
    function form( $instance ) {
        echo '<h3>Choose colours</h3>';
        echo '<div class="colours">';
        $primary = ! empty( $instance['primary'] ) ? $instance['primary'] : esc_html__( 'color', 'push2talk' );
        $secondary = ! empty( $instance['secondary'] ) ? $instance['secondary'] : esc_html__( 'color', 'push2talk' );
